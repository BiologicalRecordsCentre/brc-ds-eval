  ...
  */
  ...
  function summariesTables() {
    ...
    var generate = function generate(i) {
      if (!summary[i - 1] && dataraw$1[i - 1].json && dataraw$1[i - 1].fields) {
        d3__namespace.select("#summary-name-".concat(i)).text(dataraw$1[i - 1].name);
    ...
    var checkMap = function checkMap(i) {
      if (dataraw[i - 1].fields && (!dataraw[i - 1].fields.gr || !dataraw[i - 1].fields.taxon)) {
        ...
        if (!maps[i - 1] && dataraw[i - 1].json && dataraw[i - 1].fields) {
          ...
          var tf = dataraw[i - 1].fields.taxon;
          taxa[i - 1] = [];
          dataraw[i - 1].json.forEach(function (r) {
          ...
          taxa[i - 1].sort().forEach(function (t) {
            d3__namespace.select("#overviewmap-datalist-".concat(i)).append('option').text(t);
          });
        }

        d3__namespace.select("#overviewmap-div-".concat(i)).style("display", maps[i - 1] ? "" : "none");
        d3__namespace.select("#overviewmap-name-".concat(i)).text(dataraw[i - 1].name);
